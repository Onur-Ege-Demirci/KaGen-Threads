...
public:
    ...
    void generate_block(double* ptr, size_t size) {
        ...
        int status = vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, stream, count, ptr, 0.0, 1.0);
    ...
    void generate_int_block(int min, int max, int* output, size_t size) {
        ...
        MKL_INT count = static_cast<MKL_INT>(size);
        // VSL_RNG_METHOD_UNIFORM_STD_ACCURATE?
        int status = viRngUniform(VSL_RNG_METHOD_UNIFORM_STD, stream, count, output, min, max + 1);
        CheckVslError(status);
    }
    ...
    void generate_geometric_block(double p, int* output, size_t size) {
        check_size(size);
        MKL_INT count  = static_cast<MKL_INT>(size);
        int     status = viRngGeometric(VSL_RNG_METHOD_GEOMETRIC_ICDF, stream, count, output, p);
        CheckVslError(status);
    }
    ...
    void generate_exponential_block(double lambda, double* output, size_t size, double displacement = 0) {
        check_size(size);

        double scale = 1.0 / lambda;

        MKL_INT count  = static_cast<MKL_INT>(size);
        int     status = vdRngExponential(VSL_RNG_METHOD_EXPONENTIAL_ICDF, stream, count, output, displacement, scale);
        CheckVslError(status);
    }

    //! Generate `size` exponentially distributed integers with rate `lambda`
    //! and displacement `displacement`
    void generate_exponential_block(double lambda, std::vector<double>& output, size_t size, double displacement = 0) {
        check_size(size);
        if (size > output.size()) {
            output.resize(size);
        }
    ...
    void generate_gaussian_block(double mean, double stdev, double* output, size_t size) {
        check_size(size);
        MKL_INT count  = static_cast<MKL_INT>(size);
        int     status = vdRngGaussian(VSL_RNG_METHOD_GAUSSIAN_ICDF, stream, count, output, mean, stdev);
        CheckVslError(status);
    }

    //! Generate `size` normally distributed integers with mean `mean` and
    //! standard deviation `stdev`
    void generate_gaussian_block(double mean, double stdev, std::vector<double>& output, size_t size) {
        check_size(size);
        if (size > output.size()) {
            output.resize(size);
        }
